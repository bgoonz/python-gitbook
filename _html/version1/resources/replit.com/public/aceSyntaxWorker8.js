...
self.addEventListener('message', function(event) {
    ...
    if (changeStartRow !== null) {
        curDocument.lastValidRow = Math.min(curDocument.lastValidRow, changeStartRow);
    ...
    for (var row = highlightAll ? 0 : minRow, i = 0; row <= maxRow && i < lines.length; row++, i++) {
        if (curDocument.classifications[row] && !highlightAll) {
            ...
            // From cache
            classifications.push.apply(classifications, curDocument.classifications[row]);
            continue;
        }
        var _b = tokenizer.getLineTokens(lines[i], curDocument.states[row - 1] || 'start', row),
            tokens = _b.tokens,
            state = _b.state;
